...
static TransactionErrors parseTransactionInternalMulti(uint8_t sig_hash[][32], BigNum256 transaction_hash, bool *is_ref_out, HashState *ref_compare_hs, uint32_t number_of_inputs, uint8_t sig_hash_counter, char *change_address_ptr)
{
	...
	*is_ref_out = is_ref;
	...
	{
		#ifdef DISPLAY_PARMS
				writeEinkDisplay("TX_WRONG_VERSION ! 1", false, 10, 10, "",false,10,30, "",false,10,50, "",false,10,70, "",false,0,0);
		#endif
//		return TRANSACTION_WRONG_VERSION; // unsupported transaction version
	}

	if (readU32LittleEndian(temp) != 0x00000002)
	{
		#ifdef DISPLAY_PARMS
				writeEinkDisplay("TX_WRONG_VERSION ! 2", false, 10, 10, "",false,10,30, "",false,10,50, "",false,10,70, "",false,0,0);
		#endif
//		return TRANSACTION_WRONG_VERSION; // unsupported transaction version
	}
